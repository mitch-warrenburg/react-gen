...
const parse = (data, fp) => parseJson$1(stripBom(data), path__default['default'].relative('.', fp));

var loadJsonFile = fp => pify(gracefulFs.readFile)(fp, 'utf8').then(data => parse(data, fp));
var sync$3 = fp => parse(gracefulFs.readFileSync(fp, 'utf8'), fp);
loadJsonFile.sync = sync$3;

const filepaths = new WeakMap();
const filepath = conf => filepaths.get(conf);
const findNextCwd = pkgPath => path__default['default'].resolve(path__default['default'].dirname(pkgPath), '..');
...
const sync$4 = (namespace, opts) => {
	...
	if (opts.skipOnFalse && pkg[namespace] === false) {
		const newOpts = Object.assign({}, opts, {cwd: findNextCwd(fp)});
		return sync$4(namespace, newOpts);
	}

	return addFp(Object.assign({}, opts.defaults, pkg[namespace]), fp);
};

var pkgConf_1 = pkgConf;
var filepath_1 = filepath;
var sync_1 = sync$4;
pkgConf_1.filepath = filepath_1;
pkgConf_1.sync = sync_1;
...
var types = {
  ...
  info: {
    badge: figures_1.info,
    color: 'blue',
    label: 'info',
    logLevel: 'info'
  },
  star: {
    badge: figures_1.star,
    color: 'yellow',
    label: 'star',
    logLevel: 'info'
  },
  success: {
    badge: figures_1.tick,
    color: 'green',
    label: 'success',
    logLevel: 'info'
  },
  wait: {
    badge: figures_1.ellipsis,
    color: 'blue',
    label: 'waiting',
    logLevel: 'info'
  },
  warn: {
    badge: figures_1.warning,
    color: 'yellow',
    label: 'warning',
    logLevel: 'warn'
  },
  complete: {
    badge: figures_1.checkboxOn,
    color: 'cyan',
    label: 'complete',
    logLevel: 'info'
  },
  pending: {
    badge: figures_1.checkboxOff,
    color: 'magenta',
    label: 'pending',
    logLevel: 'info'
  },
  note: {
    badge: figures_1.bullet,
    color: 'blue',
    label: 'note',
    logLevel: 'info'
  },
  start: {
    badge: figures_1.play,
    color: 'green',
    label: 'start',
    logLevel: 'info'
  },
  pause: {
    badge: figures_1.squareSmallFilled,
    color: 'yellow',
    label: 'pause',
    logLevel: 'info'
  },
...
class Signale {
  constructor(options = {}) {
    ...
    this._types = this._mergeTypes(types, this._customTypes);
  ...
  get filename() {
    ...
    return firstExternalFilePath ? path__default['default'].basename(firstExternalFilePath) : 'anonymous';
  }

  get packageConfiguration() {
    return pkgConf_1.sync(namespace, {defaults});
  }
  ...
  _formatMessage(str) {
    return util__default['default'].format(...this._arrayify(str));
  }
  ...
  _meta() {
    ...
    if (meta.length !== 0) {
      meta.push(`${figures_1.pointerSmall}`);
  ...
  _write(stream, message) {
    if (this._interactive && stream.isTTY && isPreviousLogInteractive) {
      ...
      readline__default['default'].clearLine(stream);
...
var signale$1 = Object.assign(new signale(), {Signale: signale});

var signale$2 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.assign(/*#__PURE__*/Object.create(null), signale$1, {
    'default': signale$1
}));

const shell = require('shelljs');
const _logger = signale$2;
const logger = {
    error: (message, ...args) => {
...
const logFatalAndTerminate = (error) => {
    logger.fatal(error);
    shell.exit(1);
};
const logGeneratedContent = (templatePath, content) => {
    logger.success(`Generated the following content from template: ${path.basename(templatePath)}.
...
const logCgenConfig = (resolvedJobConfig) => {
    logger.info(`
...
const compilePath = (pathName, properties) => {
    const absolutePath = path.resolve(pathName);
    try {
        return properties ? compileTemplateString(absolutePath, properties) : absolutePath;
    }
    catch (e) {
        logger.error('Error: Unable to compile path from provided template string: %s', path__namespace);
        logFatalAndTerminate(e);
    }
};
const compileTemplateString = (template, properties = {}) => {
    try {
...
const compileTemplateFile = (templatePath, properties, pugOptions) => {
    try {
...
const resolvePaths = (pathGlobs, regexFilters) => {
    if (pathGlobs.length) {
        const findResult = shell$1.find(pathGlobs) || [];
        const filteredResult = findResult.filter(path => !regexFilters.some(exclude => path.match(exclude)));
        return Set$1(filteredResult);
    }
    return Set$1();
};
const resolveFilePath = (pathName, fileName, properties) => {
    const absoluteOutputPath = compilePath(pathName, properties);
    if (!shell$1.test('-d', absoluteOutputPath)) {
        try {
            logger.info('Output path does not exist. Creating directory at: %s', absoluteOutputPath);
            shell$1.mkdir('-p', absoluteOutputPath);
        }
        catch (e) {
            logger.error('Error: Unable to create directory from resolved path: %s', absoluteOutputPath);
            logFatalAndTerminate(e);
        }
    }
...
const writeFile = (filePath, content) => {
    try {
        ...
        shell$1.cat(filePath).to(content);
...
const mergeDeep = (objectOne, objectTwo) => {
    return Map$1(objectOne || {})
        .mergeDeep(objectTwo || {})
        .toJS();
};

const TEMPLATE_FILE_EXTENSION = '.pug';
const EMPTY_JOB_CONFIG = { fileNames: {} };
const DEFAULT_TEMPLATE_INCLUDE_PATHS = [
    '**/templates/**',
];
const DEFAULT_REACT_TEMPLATE_OUT_PATH = './src/components/#{componentName}';
const REACT_TS_CONFIG = {
    fileNames: {
        test: 'test.tsx',
        index: 'cli.ts',
        types: 'types.ts',
...
const REACT_TS_STORYBOOK_CONFIG = {
    fileNames: {
        mdx: 'mdx.ts',
        test: 'test.tsx',
        index: 'cli.ts',
...
const REACT_TS_ARCHETYPE_NAME = 'react-ts';
const REACT_TS_STORYBOOK_ARCHETYPE_NAME = 'react-ts-storybook';
const PREDEFINED_ARCHETYPES = {
...
const cgenCli = ({ parameters = [], options = {} }) => {
    const config = getCgenConfig();
    console.log(config);
    if (!config) {
        logFatalAndTerminate(new Error("Unable to locate cgen config.  Provide a cgen.config.json or add 'cgen' to your package.json."));
    }
    ...
    cgen(properties['jobName'], config, properties, cliOptions);
};
const getCgenConfig = () => {
    // if (process.env.CGEN_TEST) {
    //   process.chdir('../react-cli');
    //   return require('');
    // }
    console.log(`executing using working directory: ${process.cwd()}`);
    console.log(module.paths);
    try {
        const cgenConfig = require('cgen.config.js');
        if (!cgenConfig) {
            return cgenConfig;
        }
    }
...
const parseCliParams = (parameters) => {
    try {
...
const cgen = (jobName = '', config = {}, properties = {}, option = {}) => {
    const { base: baseJobConfig } = config, jobs = __rest(config, ["base"]);
    if (!jobs[jobName]) {
        logFatalAndTerminate(new Error('No jobs were provided.  Please provide a valid cgen job.'));
    }
...
const resolveTemplateLocations = ({ include = [], exclude = [], excludePaths = [], includePaths = DEFAULT_TEMPLATE_INCLUDE_PATHS, }) => {
...
const mergeDefaultAndSpecifiedJobs = (jobConfig = 'react-ts', baseJob = EMPTY_JOB_CONFIG) => {
    const archetype = PREDEFINED_ARCHETYPES[jobConfig];
    return archetype ? mergeDeep(baseJob, archetype) : mergeDeep(baseJob, jobConfig);
};
const generateFiles = (jobConfig, properties) => {
    const { templates } = jobConfig;
    resolveTemplateLocations(templates).map(templatePath => generateFileFromTemplate(templatePath, jobConfig, properties));
};
const generateFileFromTemplate = (templatePath, jobConfig, properties) => {
    ...
    const fileExists = shell$2.test('-f', fileOutputPath);
    if (fileExists) {
        logger.info('File %s already exists.  Skipping %s...', path.basename(templatePath), path.basename(templatePath));
    }
    else {
...
exports.cgen = cgen;
exports.cgenCli = cgenCli;
exports.generateFiles = generateFiles;
exports.getCgenConfig = getCgenConfig;
